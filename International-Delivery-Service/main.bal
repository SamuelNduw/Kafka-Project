...
kafka:ConsumerConfiguration kafkaConsumer = {
    ...
    topics: ["international_delivery_requests"]
};
...
service on new kafka:Listener(kafka:DEFAULT_URL, kafkaConsumer) {
    ...
    remote function onConsumerRecord(kafka:BytesConsumerRecord[] records) returns error? {
        foreach var consumerRecord in records {
            ...
            log:printInfo("Processing international delivery request: " + customerInfo.first_name);
            ...
            check kafkaProducer->send({
                topic: "international_delivery_responses",
